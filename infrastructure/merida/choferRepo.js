...
class ChoferRepository {
    ...
    async find() {
        let response;
        let pool;
        const queryString = "SELECT base0, dias_sucios, [$_extra_m3] FROM plantas WHERE depto = 'choferlocal' AND ciudad = 'merida';";
    ...
    async update(base, dias_sucios, extra_m3) {
        ...
        const queryString = `UPDATE plantas SET base0 = ${base} , dias_sucios = '${dias_sucios}', [$_extra_m3] = '${extra_m3}' WHERE depto = 'choferlocal' AND ciudad = 'merida';`;
    ...
    async findTeam() {
        ...
        WHERE COLABORADOR.CB_NIVEL5 = 'MDA' AND COLABORADOR.CB_ACTIVO = 'S' AND APP_NIVEL2.TB_ELEMENT = 'Ventas Distibución Local'
        `;
        try {
    ...
    async findProdDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM produccion_deptos where depto = 'Chofer Local' and city='Merida' order by id DESC
        `;
    ...
    async findBonosDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM bonos_deptos where depto = 'Chofer Local' and city='Merida' order by id DESC
        `;
