...
class AlmacenRepository {
    ...
    async findTeam() {
        ...
        WHERE COLABORADOR.CB_NIVEL5 = 'LPZ' AND COLABORADOR.CB_ACTIVO = 'S' AND APP_NIVEL2.TB_ELEMENT = 'Almacén Variable'
        `;
        try {
    ...
    async findProdDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM produccion_deptos where depto = 'Almacén' and city='La Paz' order by id DESC
        `;
    ...
    async findBonosDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM bonos_deptos where depto = 'Almacén' and city='La Paz' order by id DESC
        `;
