...
class MoldeoRepository {
    ...
    async findTeam() {
        ...
        WHERE COLABORADOR.CB_NIVEL5 = 'HMO' AND COLABORADOR.CB_ACTIVO = 'S' AND APP_NIVEL2.TB_ELEMENT = 'Moldeo Variable'
        `;
        try {
    ...
    async indicator(year, valorReal){
        ...
        const queryString = `SELECT 
                            ...
                            FROM DatoReporte 
                            WHERE IndicadorID = 50 and EntidadID = 6 and Periodo = ${year}`;
        try {
    ...
    async findBonosDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM bonos_deptos where depto = 'Moldeo' and city='Hermosillo' order by id DESC
        `;
