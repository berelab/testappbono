...
class CorteRepository {
    ...
    async find() {
        let response;
        let pool;
        const queryString = "SELECT base0, dias_sucios, [$_extra_m3] FROM plantas WHERE depto = 'corte' AND ciudad = 'nogales';";
    ...
    async update(base, dias_sucios, extra_m3) {
        ...
        const queryString = `UPDATE plantas SET base0 = ${base} , dias_sucios = '${dias_sucios}', [$_extra_m3] = '${extra_m3}' WHERE depto = 'corte' AND ciudad = 'nogales';`;
    ...
    async findTeam() {
        ...
        WHERE COLABORADOR.CB_NIVEL5 = 'NOG' AND COLABORADOR.CB_ACTIVO = 'S' AND APP_NIVEL2.TB_ELEMENT IN ('Corte Variable', 'EM Cortado')
        `;
        try {
    ...
    async indicator(year, valorReal){
        ...
        const queryString = `SELECT 
                            ...
                            FROM DatoReporte 
                            WHERE IndicadorID = 15 and EntidadID = 3 and Periodo = ${year}`;
        try {
    ...
    async findProdDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM produccion_deptos where depto = 'Corte' and city='Nogales' order by id DESC
        `;
    ...
    async findBonosDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM bonos_deptos where depto = 'Corte' and city='Nogales' order by id DESC
        `;
