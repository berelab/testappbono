...
class MantenimientoRepository {
    ...
    async find() {
        let response;
        let pool;
        const queryString = "SELECT base0, dias_sucios, [$_extra_m3] FROM plantas WHERE depto = 'mantenimiento' AND ciudad = 'tijuana';";
    ...
    async update(base, dias_sucios, extra_m3) {
        ...
        const queryString = `UPDATE plantas SET base0 = ${base} , dias_sucios = '${dias_sucios}', [$_extra_m3] = '${extra_m3}' WHERE depto = 'mantenimiento' AND ciudad = 'tijuana';`;
    ...
    async findTeam() {
        ...
        WHERE COLABORADOR.CB_NIVEL5 = 'TIJ' AND COLABORADOR.CB_ACTIVO = 'S' AND APP_NIVEL2.TB_ELEMENT = 'Mantenimiento Ind. Variable' 
        `;
        try {
    ...
    async indicatorCombustible(year, valor){
        ...
        const queryString = `
                            ...
                            ${valor[3]} as vsobresaliente
                             FROM DatoReporte where IndicadorID = 40 and EntidadID = 1 and Periodo = ${year}`;
        try {
    ...
    async indicatorAgua(year, valor){
        ...
        const queryString = `
                            ...
                            ${valor[2]} as vsatisfactorio,
                            ${valor[3]} as vsobresaliente
                             FROM DatoReporte where IndicadorID = 41 and EntidadID = 1 and Periodo = ${year}`;
        try {
            pool = await tablPoolPromise
            response = await pool.request()
            .query(queryString);
    ...
    async indicatorElectricidad(year, valor){
        ...
        const queryString = `
                            ...
                            ${valor[2]} as vsatisfactorio,
                            ${valor[3]} as vsobresaliente
                             FROM DatoReporte where IndicadorID = 39 and EntidadID = 1 and Periodo = ${year}`;
        try {
            pool = await tablPoolPromise
            response = await pool.request()
            .query(queryString);
    ...
    async findBonosDepto() {
        ...
        let pool;
        const queryString = `
        SELECT TOP (8) * FROM bonos_deptos where depto = 'Mantenimiento' and city='Tijuana' order by id DESC
        `;
